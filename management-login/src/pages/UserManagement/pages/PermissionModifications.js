...
const PermissionModifications = () => {
    ...
    const { users, userPermissions, updateUserPermissions } = useUsers();
    ...
    const handlePermissionRoleChange = (value) => setPermissionRole(value);
    const handlePermissionDepartmentChange = (value) => setPermissionDepartment(value);
    const handlePermissionYearChange = (value) => setPermissionYear(value);
    ...
    const handleApplyChanges = () => {
        ...
        // Use the context function to update permissions
        updateUserPermissions(selectedUsers, selectedPermissions);
    ...
    useEffect(() => {
        ...
        if (permissionYear !== 'Select year') {
            currentFilteredUsers = currentFilteredUsers.filter(user => String(user.year) === permissionYear);
        }
        ...
        <div className="permission-mod-page">
            ...
            <div className="filter-box">
                ...
                <div className="filter-grid">
                    <CustomSelect label="Role" options={["Select user role", "Student", "Faculty", "Staff"]} value={permissionRole} onChange={handlePermissionRoleChange} />
                    <CustomSelect label="Department" options={["Select Department", "Computer Science", "Mathematics", "Engineering", "Biology", "Arts", "Administration"]} value={permissionDepartment} onChange={handlePermissionDepartmentChange} />
                    <CustomSelect label="Year" options={generateYears()} value={permissionYear} onChange={handlePermissionYearChange} />
                </div>
            </div>
            <div className="user-table-wrapper">
                ...
                <div className="table-container">
                    <table className="data-table">
                        <thead>
                            <tr>
                                <th><input type="checkbox" onChange={(e) => { if (e.target.checked) { setSelectedUsers(filteredUsers.map(u => u.id)); } else { setSelectedUsers([]); } }} checked={selectedUsers.length > 0 && selectedUsers.length === filteredUsers.length} disabled={filteredUsers.length === 0} /></th>
                                {['Name', 'Role', 'Department', 'Year'].map(header => (<th key={header}>{header}</th>))}
                            </tr>
                        </thead>
                        <tbody>
                            {filteredUsers.length > 0 ? (
                                filteredUsers.map((user) => (
                                    <tr key={user.id} className={selectedUsers.includes(user.id) ? 'selected-row' : ''}>
                                        ...
                                        <td>{user.name}</td><td>{user.role}</td><td>{user.department}</td><td>{user.year}</td>
                                    </tr>
                                ))
                            ) : (<tr><td colSpan='5' style={{ textAlign: 'center', padding: '20px' }}>No users found matching the selected filters.</td></tr>)}
                        </tbody>
                    </table>
                </div>
            </div>
            <div className="permission-controls">
                ...
                <div className="permission-buttons">
                    {permissions.map(permission => (<button key={permission} className={`btn ${selectedPermissions.includes(permission) ? 'btn-selected' : 'btn-secondary'}`} onClick={() => togglePermissionSelection(permission)}>{permission}</button>))}
                </div>
            </div>
